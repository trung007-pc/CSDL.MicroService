...
namespace CSDL7.LanguageService.Tests;
...
public abstract class LanguageServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(CSDL7LanguageServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(CSDL7LanguageServiceModule).Assembly.FullName} assembly!");
        }
