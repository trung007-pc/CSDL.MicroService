...
+   metrics_path: 'metrics'
+   static_configs:
+   - targets: ['host.docker.internal:44328']
```

## Creating Helm Chart for the EmailService
...
+ ./build-image.ps1 -ProjectPath "../../services/email/CSDL7.EmailService/CSDL7.EmailService.csproj" -ImageName csdl7/email
```
...
global:
  ...
  connectionStrings:
    ...
+   email: "Server=[RELEASE_NAME]-sqlserver,1433; Database=CSDL7_Email; User Id=sa; Password=myPassw@rd; TrustServerCertificate=true; Connect Timeout=240;"
```
...
Email microservice `values.yaml` file. 

```yaml
image:
  repository: "csdl7/email"
  tag: "latest"
...
Email microservice `Chart.yaml` file. 
...
version: 1.0.0
appVersion: "1.0"
description: CSDL7 Email Service
```

Email microservice `email.yaml` file. 
    ...
    spec:
      ...
      - image: "{{ .Values.image.repository }}:{{ .Values.image.tag }}"
        ...
        - name: "ConnectionStrings__EmailService"
          value: "{{ .Values.global.connectionStrings.email | replace "[RELEASE_NAME]" .Release.Name }}"
          ...
```

Email microservice `email-service.yaml` file. 
...
You can *Refresh Sub Charts* in ABP Studio. Don't forget to add the following *Metadata* information to the sub-chart's *Metadata* keys. You can access this menu by opening the *Kubernetes* menu, right-clicking the *email* sub-chart, and selecting *Properties* -> *Metadata* tab.

- `projectPath`: **../../services/email/CSDL7.EmailService/CSDL7.EmailService.csproj**
- `imageName`: **csdl7/email**
- `projectType`: **dotnet**
  ...
  template:
    ...
    spec:
      ...
      - image: "{{ .Values.image.repository }}:{{ .Values.image.tag }}"
        ...
        env:
        ...
+       - name: "OpenIddict__Resources__EmailService__RootUrl"
+         value: "http://{{ .Release.Name }}-email"
```
...
+         value: "...,http://{{ .Release.Name }}-administration,http://{{ .Release.Name }}-email"
```
...
```bash
abp generate-proxy -t csharp -url http://localhost:44328/ -m email --without-contracts
```
...
```bash
abp generate-proxy -t ng -url http://localhost:44328/ -m email
```