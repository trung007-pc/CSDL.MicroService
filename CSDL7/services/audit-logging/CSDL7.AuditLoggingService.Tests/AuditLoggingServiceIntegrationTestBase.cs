...
public abstract class AuditLoggingServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(CSDL7AuditLoggingServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(CSDL7AuditLoggingServiceModule).Assembly.FullName} assembly!");
        }
